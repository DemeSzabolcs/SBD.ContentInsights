    ...
    : INotificationAsyncHandler<UmbracoApplicationStartingNotification>
{
    ...
    private readonly Guid SuperUserKey = Umbraco.Cms.Core.Constants.Security.SuperUserKey;
    ...
    public async Task HandleAsync(UmbracoApplicationStartingNotification notification, CancellationToken cancellationToken)
    {
        ...
        if (userGroup != null)
        {
            _logger.LogInformation($"SBD.ContentInsights - User group " +
        ...
        if (attempt.Success)
        {
            _logger.LogInformation($"SBD.ContentInsights - Created user group " +
            ...
            if (updateAttempt.Success)
            {
                ...
                if (adminGroup == null)
                {
                    _logger.LogWarning("SBD.ContentInsights - There is no \"admin\" group, the " +
            ...
            else
            {
                _logger.LogError(
                    ...
                    $"SBD.ContentInsights - Failed to update user group {Roles.ContentInsightsAdministratorsGroup}.");
            }
        }
        else
        {
            _logger.LogError(
                attempt.Exception,
                $"SBD.ContentInsights - Failed to create user group {Roles.ContentInsightsAdministratorsGroup}.");
        }
    }
}
